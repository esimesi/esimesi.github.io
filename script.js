// Maintain a CSS variable for sticky offset and use scrollIntoView for robust scrolling
function setStickyOffsetVar() {
  const stickyBar = document.querySelector('.filter-bar-nav');
  const offset = (stickyBar ? stickyBar.offsetHeight : 0) + 8; // small gap
  document.documentElement.style.setProperty('--sticky-offset', offset + 'px');
}

// Smooth scroll to the heading (where items start)
function scrollToHeading() {
  const heading = document.querySelector('.heading');
  if (!heading) {
    console.warn('No .heading element found for scrollToHeading');
    return;
  }
  // Ensure the CSS var is fresh (in case height changed)
  setStickyOffsetVar();
  if ('scrollIntoView' in heading) {
    heading.scrollIntoView({ behavior: 'smooth', block: 'start' });
  } else {
    // Fallback: compute position and clamp to document height (prevents bottom cutoff on small screens)
    const stickyBar = document.querySelector('.filter-bar-nav');
    const offset = (stickyBar ? stickyBar.offsetHeight : 0) + 8;
...
// Filter bar arrow scroll logic
document.addEventListener('DOMContentLoaded', function () {
  // Initialize sticky offset var early
  setStickyOffsetVar();
  ...
  if (!filterBar || !leftArrow || !rightArrow) return;

  function updateArrows() {
  const canScrollLeft = filterBar.scrollLeft > 0;
  const canScrollRight = (filterBar.scrollLeft + filterBar.clientWidth) < (filterBar.scrollWidth - 1);

  // Disable and hide when not applicable
  leftArrow.disabled = !canScrollLeft;
  rightArrow.disabled = !canScrollRight;
  leftArrow.hidden = !canScrollLeft;
  rightArrow.hidden = !canScrollRight;
  leftArrow.setAttribute('aria-hidden', String(!canScrollLeft));
  rightArrow.setAttribute('aria-hidden', String(!canScrollRight));
  }

  leftArrow.addEventListener('click', () => {
    filterBar.scrollBy({ left: -150, behavior: 'smooth' });
  });
...
const filterButtons = document.querySelectorAll('.filter-bar .filter-chip');
...
const pizzaBtn = document.querySelector('.filter-bar .filter-chip[data-filter="pizza"]');
if (pizzaBtn) pizzaBtn.classList.add('active');
...
filterButtons.forEach(btn => {
  btn.addEventListener('click', () => {
    console.log('Filter chip clicked:', btn.dataset.filter);
    filterButtons.forEach(btn => {
  ...
  item2s.forEach(item2 => {
    ...
    // Smoothly scroll to the heading (where items start) after filter is applied
    setTimeout(() => {
      scrollToHeading();
    }, 120); // allow for UI update
...
function updateHeaderVisibility() {
  ...
  var activeCategoryBtn = document.querySelector('.filter-bar .filter-chip.active');
  var activeCategory = activeCategoryBtn ? activeCategoryBtn.dataset.filter : null;
  ...
  pizzaHeader.style.display = 'flex';
    } else {
...
// Scroll helper: scroll to first visible menu item beneath the heading, compensating for sticky filter bar
// (Reverted) No longer scrolling to first visible item; we scroll to heading only
...
specialButtonForDagensLunch.addEventListener('click', (event) => {
  ...
  // Trigger the filter chip click directly - this will execute all the filtering logic
  if (dagensLunchFilterButton) {
    dagensLunchFilterButton.click();
  }

  // Optional: Add a small delay before scrolling to ensure filtering is complete
  setTimeout(() => {
    scrollToHeading(); // Use the existing scrollToHeading function for consistency
  }, 200); // Shorter delay since we're using the built-in scroll function
});
